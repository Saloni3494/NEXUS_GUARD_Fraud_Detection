...
const hex = (s: number) => s >= 0.75 ? "#ef4444" : s >= 0.45 ? "#facc15" : "#06b6d4";
const tc  = (s: number) => s >= 0.75 ? "text-red-400" : s >= 0.45 ? "text-yellow-400" : "text-brand-cyan";
const bg  = (s: number) =>
  s >= 0.75 ? "bg-red-500/[0.04] border-red-500/20"
  : s >= 0.45 ? "bg-yellow-400/[0.04] border-yellow-400/20"
  : "bg-brand-cyan/[0.04] border-brand-cyan/20";
const dc  = (d: string) =>
...
function Bar({ label, value, max = 1, color = "#06b6d4" }: { label: string; value: number; max?: number; color?: string }) {
...
function StatCard({ label, value, sub, color = "text-brand-cyan", accent }: { label: string; value: React.ReactNode; sub: string; color?: string; accent?: string }) {
  return (
...
function LiveBadge({ loading, error }: { loading: boolean; error: boolean }) {
  ...
  return (
    <span className="flex items-center gap-1.5 text-[9px] text-brand-cyan/70 font-mono font-bold">
      <span className="w-1.5 h-1.5 rounded-full bg-brand-cyan" style={{ boxShadow: "0 0 6px #06b6d4" }} />
      live
    </span>
  );
}
...
function Canvas({ liveNodes }: { liveNodes?: LiveNode[] }) {
  ...
  useEffect(() => {
    ...
    pts.current = source.map(n => ({
      ...
      color: n.ring ? "#f97316" : n.is_fraud ? "#ef4444" : n.risk > 0.5 ? "#facc15" : "#06b6d4",
...
function PipeStep({ n, label, active, done, tag }: { n: number; label: string; active: boolean; done: boolean; tag?: string }) {
  return (
    <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${active ? "bg-brand-cyan/[0.09] border border-brand-cyan/20" : "border border-transparent"}`}>
      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black shrink-0 transition-all ${done ? "bg-brand-cyan text-black" : active ? "bg-brand-cyan text-black" : "bg-white/[0.06] text-white/70 border border-white/[0.1]"}`}>
        {done ? "✓" : n}
      </div>
      <span className={`text-[11px] flex-1 transition-colors leading-tight font-semibold ${active ? "text-brand-cyan" : done ? "text-brand-cyan/50" : "text-white/60"}`}>{label}</span>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input value={form[k]} onChange={e => setForm((f: any) => ({ ...f, [k]: e.target.value }))}
        className="w-full bg-white/[0.04] border border-white/[0.1] hover:border-white/[0.18] focus:border-brand-cyan/40 rounded-xl px-4 py-2.5 text-sm text-white font-mono placeholder:text-white/50 focus:outline-none transition-colors" />
    </div>
  );
}
    ...
    <div className="mb-6 sm:mb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
      <div className="flex-1">
        ...
        <h2 className="text-2xl sm:text-[2.6rem] font-black tracking-tight leading-none mb-2 sm:mb-3">
          {title} <span className="text-brand-cyan">{accent}</span>
        </h2>
...
function SimulatorSection() {
  ...
  const fusionComponents = result ? [
    { w: 0.40, v: result.gnnScore,      label: "GNN",      color: "#06b6d4"  },
    ...
    <div className="flex flex-col gap-4 lg:grid lg:grid-cols-[270px_1fr_210px] lg:gap-5 lg:h-full">
      ...
      <Card className="flex flex-col">
        ...
        <div className="p-5 sm:p-6 space-y-5 flex-1">
          <div className="space-y-3">
            <div className="px-3 py-2 rounded-lg bg-brand-cyan/[0.04] border border-brand-cyan/10">
              <p className="text-[9px] text-brand-cyan/70 leading-relaxed">
        ...
        <div className="p-4 sm:p-5 border-t border-white/[0.04]">
          <button onClick={run} disabled={loading}
            className="w-full py-3.5 bg-brand-cyan hover:bg-brand-cyan/80 active:scale-[0.99] disabled:opacity-40 text-black font-bold rounded-xl text-[11px] uppercase tracking-[0.15em] transition-all">
              ...
              <div className="flex gap-1.5 mb-6 pb-5 border-b border-white/[0.05] flex-wrap">
                {["Overview","Behavioral","Structural","Identity","Fusion"].map(t => (
                  <button key={t} onClick={() => setTab(t)}
                    className={`px-3 sm:px-4 py-2 rounded-full text-[10px] sm:text-[11px] font-bold uppercase tracking-wider transition-all ${tab === t ? "bg-brand-cyan text-black" : "text-white/75 hover:text-white"}`}>
                    {t}
                  </button>
                ))}
              </div>
              ...
              {tab === "Structural" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-10">
                  ...
                  <div>
                    ...
                    <div className={`p-5 sm:p-6 rounded-2xl border ${result.muleRing.isMuleRingMember ? "bg-red-500/[0.03] border-red-500/15" : "bg-brand-cyan/[0.03] border-brand-cyan/15"}`}>
                      <p className={`text-2xl font-black mb-5 ${result.muleRing.isMuleRingMember ? "text-red-400" : "text-brand-cyan"}`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      ...
                      <Card key={l} className="p-5">
                        ...
                        <p className={`text-4xl font-black font-mono mb-4 ${(v ?? 0) > w ? "text-red-400" : "text-brand-cyan"}`}>
                          {fmt ? fmt(v ?? 0) : (v ?? 0)}
                        </p>
                        <Bar label="signal" value={Math.min(1, (v ?? 0) / 10)} color={(v ?? 0) > w ? "#ef4444" : "#06b6d4"} />
                      </Card>
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-4 sm:gap-8 p-5 rounded-xl border border-white/[0.05]">
                    {([["New Device", result.ja3.isNewDevice], ["New JA3", result.ja3.isNewJa3]] as [string, boolean][]).map(([l, v]) => (
                      <div key={l} className="flex gap-3 items-center">
                        ...
                        <Pill className={v ? "bg-yellow-400/10 border border-yellow-400/20 text-yellow-400" : "bg-brand-cyan/10 border border-brand-cyan/20 text-brand-cyan"}>{v ? "YES" : "NO"}</Pill>
                      </div>
                    ))}
                  </div>
      ...
      <div className="lg:block">
        ...
        <Card className={`flex-col overflow-y-auto ${showPipeline ? "flex" : "hidden lg:flex"}`}>
          ...
          {step === 15 && (
            <div className={`m-3 p-3.5 rounded-xl border ${apiError ? "bg-red-500/[0.07] border-red-500/15" : "bg-brand-cyan/[0.07] border-brand-cyan/15"}`}>
              <p className={`text-[11px] font-bold ${apiError ? "text-red-400" : "text-brand-cyan"}`}>
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
        {[
          ...
          { l: "Layer 2", t: "GATConv ×4",  d: "Attention-weighted neighbour selection",       c: "text-brand-cyan",  b: "border-brand-cyan/[0.08]" },
        ...
        <Card className="p-5 sm:p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {[
              ...
              <div key={f} className="p-3 rounded-xl border border-white/[0.04] bg-white/[0.01] hover:border-brand-cyan/15 transition-colors group cursor-default">
                <p className="text-[9px] font-bold text-brand-cyan/70 leading-tight mb-0.5 group-hover:text-brand-cyan/90 transition-colors">{f}</p>
        ...
        <div className="flex flex-col gap-4">
          <Card className="p-5 flex-1">
            ...
            <div className="flex flex-wrap gap-3 sm:gap-5 mt-4">
              {[["#ef4444","Fraud"],["#f97316","Ring member"],["#facc15","High-risk"],["#06b6d4","Safe"]].map(([c, l]) => (
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-6 p-4 rounded-2xl border border-white/[0.07] bg-white/[0.02]">
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${health?.model_loaded ? "bg-brand-cyan" : "bg-red-400"}`}
            style={health?.model_loaded ? { boxShadow: "0 0 8px #06b6d4" } : {}} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <Card className="p-5 sm:p-7">
          ...
          <div className="space-y-3">
            {[
              { n:"1", t:"6 Raw Features",        d:"velocity_score, burst_score, suspicious_neighbor_count, ja3_reuse_count, device_reuse_count, ip_reuse_count — sent from Spring Boot", c:"#06b6d4" },
        ...
        <Card className="p-5 sm:p-7">
          ...
          <div className="space-y-3">
            {[
              ...
              { l:"Device Reuse Count",         v:0.28, c:"#06b6d4", d:"device hash seen across N accounts" },
              { l:"IP Reuse Count",             v:0.18, c:"#06b6d4", d:"IP shared across N accounts" },
            ].map(f => (
...
function IdentitySection() {
  ...
  const signals = [
    {
      t: "JA3 Fingerprint", color: "#06b6d4",
  ...
  return (
    <div>
      ...
      <div className={`mb-6 p-4 rounded-2xl border flex items-start sm:items-center gap-3 ${hasLive ? "border-brand-cyan/15 bg-brand-cyan/[0.04]" : "border-white/[0.07] bg-white/[0.02]"}`}>
        <div className={`w-2 h-2 rounded-full shrink-0 mt-1 sm:mt-0 ${hasLive ? "bg-brand-cyan" : "bg-white/30"}`}
          style={hasLive ? { boxShadow: "0 0 8px #06b6d4" } : {}} />
        <p className={`text-sm font-medium ${hasLive ? "text-brand-cyan/80" : "text-white/65"}`}>
          {hasLive
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { n:"1", t:"JA3 Fingerprint",    d:"TLS ClientHello hash — identifies the SSL library regardless of IP. One malicious tool = one fingerprint, even across thousands of accounts.", c:"#06b6d4" },
...
function FusionSection() {
  ...
  const components = [
    { l:"GNN Score",      v: hasLive ? last!.gnnScore      : 0.847, w:0.40, c:"#06b6d4", d:"Graph Neural Network — structural fraud patterns" },
  ...
  return (
    <div>
      ...
      <div className={`mb-6 p-4 rounded-2xl border flex flex-col sm:flex-row sm:items-center gap-3 ${hasLive ? "border-brand-cyan/15 bg-brand-cyan/[0.04]" : "border-white/[0.07] bg-white/[0.02]"}`}>
        <div className={`w-2 h-2 rounded-full shrink-0 ${hasLive ? "bg-brand-cyan" : "bg-white/30"}`}
          style={hasLive ? { boxShadow: "0 0 8px #06b6d4" } : {}} />
        <p className={`text-sm font-medium ${hasLive ? "text-brand-cyan/80" : "text-white/65"}`}>
            ...
            {rings.map((ring: any, i: number) => {
              ...
              return (
                <div key={i} onClick={() => setSel(isSel ? null : ring)}
                  className={`p-4 rounded-xl border cursor-pointer transition-all ${isSel ? "bg-brand-cyan/[0.05] border-brand-cyan/20" : "border-white/[0.07] hover:border-white/[0.12]"}`}>
                ...
                <tbody>
                  {clusters.map((c: any, i: number) => (
                    <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.015] transition-colors">
                      <td className="px-4 sm:px-5 py-4 text-brand-cyan font-mono text-sm font-bold">{c.node_id}</td>
                      <td className="px-4 sm:px-5 py-4">
                        <Pill className={c.is_fraud ? "bg-red-500/10 border border-red-500/15 text-red-400" : "bg-brand-cyan/10 border border-brand-cyan/15 text-brand-cyan"}>
                          {c.is_fraud ? "YES" : "NO"}
                        </Pill>
                      </td>
                      <td className="px-4 sm:px-5 py-4">
        ...
        <div className="flex flex-col gap-4">
          <Card className="p-5 sm:p-6 flex-1">
            ...
            {([
              ...
              ["Low (<10%)",      Math.round(total * 0.55),   "#06b6d4"],
          ...
          <Card className="p-5 bg-brand-cyan/[0.02] border-brand-cyan/[0.07]">
            <p className="text-[10px] text-brand-cyan/70 leading-relaxed italic">"Nodes inside a high-fraud cluster inherit elevated suspicion during GNN message passing via the community_fraud_rate feature."</p>
          </Card>
        </div>
      </div>
    </div>
...
function BlockchainSection() {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
        <StatCard label="Status"       value="LIVE"      sub="50 events / 5s batching"    accent="#06b6d4" />
        ...
        <div className="flex flex-col gap-4">
          <Card className="p-5 sm:p-6">
            ...
            <div className="p-4 rounded-xl bg-black/50 border border-white/[0.07] font-mono text-[11px] space-y-1.5 overflow-x-auto">
              <p className="text-brand-cyan font-black">MERKLE ROOT: 0xe4f2…3b91</p>
          ...
          <Card className="p-5 sm:p-6 flex-1">
            ...
            <div className="space-y-3.5">
              {([
                ["1","Decision committed to MongoDB","#06b6d4"],
                ...
                ["5","Root written to blockchain","#06b6d4"],
          ...
          {stats && (
            <Card className="p-5 bg-brand-cyan/[0.025] border-brand-cyan/[0.09]">
              ...
              {([
                ...
                <div key={k} className="flex justify-between py-2 border-b border-white/[0.05] last:border-0">
                  ...
                  <span className="text-[11px] font-black text-brand-cyan/90 font-mono">{v}</span>
                </div>
              ))}
            </Card>
          )}
  ...
  const MODELS = [
    {
      ...
      color: "#06b6d4", border: "border-brand-cyan/[0.1]", activeBg: "bg-brand-cyan/[0.07]",
      ...
      <PageHeading eyebrow="Evaluation Results" title="Model" accent="Performance"
        ...
        action={
          <a href={`${API_URL}/api/admin/evaluate-models/download`} download
             className="flex items-center gap-2.5 px-5 py-3 rounded-2xl bg-brand-cyan text-black font-black text-[11px] uppercase tracking-[0.15em] hover:bg-brand-cyan/90 transition-all shadow-[0_0_20px_rgba(202,255,51,0.25)] hover:shadow-[0_0_30px_rgba(202,255,51,0.45)] active:scale-95 group">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            ...
            {active.trainAuc > 0 && (
               <div className="mt-5 p-4 rounded-xl border border-white/[0.04] bg-white/[0.01]">
                 <div className="flex justify-between items-center mb-1">
                    ...
                    <span className={`text-base font-black font-mono ${active.trainAuc > 0.95 ? "text-brand-cyan" : "text-white"}`}>{f4(active.trainAuc, 4)}</span>
                 </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-5">
              ...
              <span className={`text-[8px] px-1.5 py-0.5 rounded font-black tracking-widest uppercase ${active.live ? "bg-brand-cyan/20 text-brand-cyan" : "bg-white/10 text-white/60"}`}>
              ...
              {active.id === "gnn" && gnnAuc > 0 && (
                <>
                  ...
                  <div className="flex gap-2 pt-2 flex-wrap">
                    {gnnF1 > 0.8  && <Pill className="bg-brand-cyan/10 border border-brand-cyan/20 text-brand-cyan">F1 &gt; 0.80 ✓</Pill>}
                    {gnnAuc > 0.9 && <Pill className="bg-brand-cyan/10 border border-brand-cyan/20 text-brand-cyan">AUC &gt; 0.90 ✓</Pill>}
                  </div>
                </>
              )}
            </div>
            ...
            {active.id === "gnn" && active.confMatrix && (
              <div className="mt-5 pt-5 border-t border-white/[0.04]">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {[
                    { l:"TN", sub:"correct legit", v:active.confMatrix[0]?.[0]??0, c:"text-brand-cyan"  },
                    ...
                    { l:"TP", sub:"caught fraud",  v:active.confMatrix[1]?.[1]??0, c:"text-brand-cyan"  },
                  ].map(({ l, sub, v, c }) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-4">
          <Card className="p-5 sm:p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { l: "GNN",          v: modelStatus, c: modelStatus === "HEALTHY" ? "#06b6d4" : "#ef4444" },
                { l: "EIF",          v: "ACTIVE",    c: "#a855f7" },
                { l: "Nodes",        v: nodesCount > 0 ? nodesCount.toLocaleString() : "—", c: "#06b6d4" },
                { l: "Rings cached", v: ringsCached > 0 ? String(ringsCached) : "—", c: "#06b6d4" },
              ].map(({ l, v, c }) => (
    ...
    <>
      <div className="mb-7 px-3">
        <div className="flex items-center gap-2.5 mb-1">
          <div className="w-6 h-6 rounded-lg bg-brand-cyan/15 border border-brand-cyan/25 flex items-center justify-center">
            <div className="w-2.5 h-2.5 rounded-sm bg-brand-cyan" />
          </div>
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/70">NexusGuard AI</p>
            ...
            <div className="space-y-0.5">
              {group.items.map(({ id, label, icon: Icon }) => (
                <button key={id} onClick={() => { setActive(id); setSidebarOpen(false); }}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all duration-150 group ${active === id ? "bg-brand-cyan/[0.1] border border-brand-cyan/20" : "border border-transparent hover:bg-white/[0.03] hover:border-white/[0.06]"}`}>
                  <Icon className={`w-4 h-4 shrink-0 transition-colors ${active === id ? "text-brand-cyan" : "text-white/65 group-hover:text-white/80"}`} />
                  <span className={`text-[12px] font-semibold tracking-tight ${active === id ? "text-brand-cyan" : "text-white/70 group-hover:text-white/90"}`}>{label}</span>
                  {active === id && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-brand-cyan" style={{ boxShadow: "0 0 6px #06b6d4" }} />}
                </button>
              ))}
            </div>
          </div>
  ...
  return (
    ...
    <div className="bg-[#060606] text-white min-h-screen flex flex-col">
      ...
      <div className="lg:hidden sticky top-0 z-40 flex items-center justify-between px-4 py-3 bg-[#070707] border-b border-white/[0.07]">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded-md bg-brand-cyan/15 border border-brand-cyan/25 flex items-center justify-center">
