  ...
  return (
    <section className="py-20 px-8">
      <div className="flex justify-between items-end mb-12">
        <div>
          <h2 className="text-4xl text-white font-medium">Our <span className="text-brand-cyan">Testimonials</span></h2>
        ...
        <div className="bg-[#1A1A1A] p-1 rounded-full border border-gray-800 flex">
          <button className="bg-brand-cyan text-black px-6 py-2 rounded-full text-sm font-medium">For Individuals</button>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {reviews.map((rev, i) => (
          <div key={i} className="text-center">
             <div className="text-brand-cyan text-5xl mb-4">"</div>
             <p className="text-gray-300 text-sm leading-relaxed mb-6">{rev.text}</p>
             <p className="text-brand-cyan font-medium">{rev.name}</p>
          </div>
        ))}
      </div>
    </section>
