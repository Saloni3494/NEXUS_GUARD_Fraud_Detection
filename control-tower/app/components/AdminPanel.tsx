      ...
      <div className="px-10 pt-8 flex justify-between items-end">
        <div>
          ...
          <p className="text-xl font-medium">
            {session?.user?.name} <span className="text-brand-cyan text-xs ml-2">● ONLINE</span>
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-6 p-6 md:p-10 overflow-hidden">
        ...
        <section className="w-full md:w-1/3 bg-[#1C1C1C] p-6 rounded-2xl border border-gray-800 shadow-xl flex flex-col justify-center">
          <header className="mb-6">
            <h1 className="text-brand-cyan text-2xl font-semibold">System Access</h1>
          ...
          <form action={clientAction} className="space-y-5">
                ...
                {roles.map((r) => (
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-medium rounded-md transition-all ${
                      role === r ? "bg-[#262626] text-brand-cyan border border-gray-700" : "text-gray-500"
                    }`}
                  >
                    {r}
                  </button>
            ...
            <div className="space-y-4">
              <input 
                ...
                required
                className="w-full bg-[#141414] border border-gray-800 p-3 rounded-lg text-sm focus:border-brand-cyan outline-none transition-all placeholder:text-gray-600" 
              />
              <input 
                name="email" 
                type="email" 
            ...
            <button 
              type="submit" 
              disabled={isPending}
              className="w-full bg-brand-cyan cursor-pointer hover:bg-[#afd149] py-3 rounded-lg font-bold text-black text-sm hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
