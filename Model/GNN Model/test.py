...
graph_path = SHARED / "processed_graph.pt"
...
check("graph loaded", data is not None)
...
safe_count  = int(class_counts[0])
fraud_count = int(class_counts[1])

check(
    "has fraud labels",
    fraud_count > 0,
    f"{fraud_count} fraud nodes"
)

fraud_rate = fraud_count / len(data.y)
...
check(
    "train split contains fraud",
    train_fraud > 0,
    f"{train_fraud} fraud nodes"
)

check(
    "val split contains fraud",
    val_fraud > 0,
    f"{val_fraud} fraud nodes"
)

check(
    "test split contains fraud",
    test_fraud > 0,
    f"{test_fraud} fraud nodes"
)

print(f"\n  Nodes          : {data.num_nodes:,}")
print(f"  Edges          : {data.num_edges:,}")
print(f"  Features       : {data.num_features:,}")
print(f"  Safe Nodes     : {safe_count:,}")
print(f"  Fraud Nodes    : {fraud_count:,}")
print(f"  Train Fraud    : {train_fraud}")
print(f"  Val Fraud      : {val_fraud}")
print(f"  Test Fraud     : {test_fraud}")

# Dummy IDs for later API tests
real_node_id  = "0"
fraud_node_id = "1"
safe_node_id  = "2"

norm = {
    "feature_cols": [f"F{i}" for i in range(data.num_features)],
    "col_min": [0] * data.num_features,
    "col_max": [1] * data.num_features,
    "col_range": [1] * data.num_features,
}

check(
    "feature_cols generated",
    len(norm["feature_cols"]) == data.num_features,
    f"{len(norm['feature_cols'])} cols"
)
